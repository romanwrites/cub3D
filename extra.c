...
void		draw_сircle_filled(int xm, int ym, int r, t_game *sv, unsigned int *colours)
{
	...
	do {
		...
		my_mlx_pixel_put(sv, xm-x, ym+y, colour); /*   I. Quadrant */
		my_mlx_pixel_put(sv, xm-x, ym-y, colour); /*  II. Quadrant */
		my_mlx_pixel_put(sv, xm+x, ym-y, colour); /* III. Quadrant */
		my_mlx_pixel_put(sv, xm+x, ym+y, colour); /*  IV. Quadrant */
		draw_line_bresenham(xm-x, ym+y, xm-x, ym-y, colour, sv); //draw line
		draw_line_bresenham(xm+x, ym-y, xm+x, ym+y, colour, sv); //kek
//		if (colours[1] == 255)
//			colours[1] = 0;
//		colours[1] -= 1;
//		colours[2] -= 1;
