...
void		set_sprites_coordinates(t_game *sv)
{
	...
	i = 0;
	j = 0;
	while (sv->map.map_arr[i])
	{
		if (sv->map.map_arr[i] == '2')
		{
...
void		create_map_array(t_game *sv)
{
	...
	j = 0;
	ft_alloc_check(sv->map.map_arr = ft_calloc(1, \
	...
	while (sv->lst)
	{
		...
		{
			sv->map.map_arr[j] = ((char *)sv->lst->content)[i];
			if (sv->map.map_arr[j] == 'N' || sv->map.map_arr[j] == 'S' || \
				sv->map.map_arr[j] == 'W' || sv->map.map_arr[j] == 'E')
				set_player_vectors(sv, j, i);
			if (sv->map.map_arr[j] == '2')
				sv->sprites_count++;
			i++;
			j++;
		}
